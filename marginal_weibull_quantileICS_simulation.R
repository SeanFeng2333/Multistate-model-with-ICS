...
if (length(args)==0){
  ...
  ## users can adjust the following numbers based on how many simulations they need
...
simulation.func <- function(x){
  ...
  for (s in 1:numsim){
    ...
    out <- tryCatch({  
      ...
      cluster_size_grp <- c()
      for (i in 1:m) {
        if (frailty[i] <= quantile(w, 0.4)) { cluster_size[i] = rbinom(1, 27, 0.8)+1 }
        else if   (frailty[i] > quantile(w, 0.4) & frailty[i] <= quantile(w, 0.8)) {
          cluster_size[i] = rbinom(1, 27, 0.5)+1 }
        else {cluster_size[i] = rbinom(1, 27, 0.2)+1 }}
      
      for (i in 1:m) {
        if (cluster_size[i] <= 10) { cluster_size_grp[i] = 1 }
        else if (cluster_size[i] > 11 & cluster_size[i] <= 20) { cluster_size_grp[i] = 2 }
        else { cluster_size_grp[i] = 3}
      }
      ...
      #exact.t.23 <- ( -log(1-u23) / (baseline_haz23 * frailty_vec * exp(beta2_23 * x2)))^(1/gamma)  + exact.t.12
      ...
      #### naive ####
      # specify inimtial intensity matrix 
      ...
      naive.model <- msm(state ~ visit.time, 
                         ...
                         opt.method="optim",method="BFGS",  # "BFGS" quasi-Newton optimization algorithm
                         hessian=T,
                         qmatrix=twoway3.q)
      
      naive.est <- naive.model$estimates
      # naive.est <- c(exp(naive.est[[1]]), exp(naive.est[[2]]), naive.est[[3]], naive.est[[4]])
...
colnames(wsf.weibull.pw.est.result) <- c("log.gamma.t1", "log.lambda120.t1", "log.lambda230.t1", "beta12.t1", "beta23.t1", "log.gamma.t2", "log.lambda120.t2", "log.lambda230.t2", "beta12.t2", "beta23.t2")
colnames(wsf.weibull.pw.se.result) <- c("log.gamma.t1", "log.lambda120.t1", "log.lambda230.t1", "beta12.t1", "beta23.t1", "log.gamma.t2", "log.lambda120.t2", "log.lambda230.t2", "beta12.t2", "beta23.t2")

if (task_id==1){
  write.csv(naive.exp.est.result, paste0("naive.exp.est_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = T)
  write.csv(naive.exp.se.result, paste0("naive.exp.se_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = T)
  write.csv(naive.weibull.est.result, paste0("naive.weibull.est_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = T)
  write.csv(naive.weibull.se.result, paste0("naive.weibull.se_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = T)
  write.csv(wcr.weibull.est.result, paste0("wcr.weibull.est_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = T)
  write.csv(wcr.weibull.se.result, paste0("wcr.weibull.se_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = T)
  write.csv(wsf.weibull.est.result, paste0("wsf.weibull.est_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = T)
  write.csv(wsf.weibull.se.result, paste0("wsf.weibull.se_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = T)
  write.csv(wsf.weibull.pw.est.result, paste0("wsf.weibull.pw.est_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = T)
  write.csv(wsf.weibull.pw.se.result, paste0("wsf.weibull.pw.se_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = T)
}else {
  write.csv(naive.exp.est.result, paste0("naive.exp.est_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = F)
  write.csv(naive.exp.se.result, paste0("naive.exp.se_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = F)
  write.csv(naive.weibull.est.result, paste0("naive.weibull.est_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = F)
  write.csv(naive.weibull.se.result, paste0("naive.weibull.se_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = F)
  write.csv(wcr.weibull.est.result, paste0("wcr.weibull.est_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = F)
  write.csv(wcr.weibull.se.result, paste0("wcr.weibull.se_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = F)
  write.csv(wsf.weibull.est.result, paste0("wsf.weibull.est_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = F)
  write.csv(wsf.weibull.se.result, paste0("wsf.weibull.se_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = F)
  write.csv(wsf.weibull.pw.est.result, paste0("wsf.weibull.pw.est_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = F)
  write.csv(wsf.weibull.pw.se.result, paste0("wsf.weibull.pw.se_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, "task_id", task_id, ".csv"), row.names =F, col.names = F)
}
...
write.csv(sim_final_result, paste0("marginal.result_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, ".csv"),  row.names =T)
write.csv(sim_final_result_wsf.pw, paste0("wsf.pw.result_m", m, "quantileICS", "tau", tau, "gamma", gamma, "visit", visittimes, ".csv"),  row.names =T)
